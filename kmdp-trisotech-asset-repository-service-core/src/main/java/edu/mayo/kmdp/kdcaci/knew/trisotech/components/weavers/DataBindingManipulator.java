...
import static edu.mayo.kmdp.kdcaci.knew.trisotech.TTConstants.DMN_13_XMLNS;
import static edu.mayo.kmdp.kdcaci.knew.trisotech.TTConstants.DMN_14_XMLNS;
import static edu.mayo.kmdp.kdcaci.knew.trisotech.TTConstants.TT_INPUT_BINDINGS;
import static edu.mayo.kmdp.kdcaci.knew.trisotech.TTConstants.TT_METADATA_NS;
import static edu.mayo.kmdp.kdcaci.knew.trisotech.TTConstants.TT_OUTPUT_BINDINGS;
import static edu.mayo.kmdp.util.StreamUtil.filterAs;
import static edu.mayo.kmdp.util.XMLUtil.asElementStream;

import edu.mayo.kmdp.kdcaci.knew.trisotech.TTConstants;
...
public class DataBindingManipulator {
   ...
   */
  public static void rewriteInputDataBindings(Document dox) {
    // the triso:InputBindings are the proprietary elements
    asElementStream(dox.getElementsByTagNameNS(TT_METADATA_NS, TT_INPUT_BINDINGS))
   ...
   * @param dox The CMMN model to rewrite, as an XML DOM
   */
  public static void rewriteOutputDataBindings(Document dox) {
    // the triso:OutputBindings are the proprietary elements
    asElementStream(dox.getElementsByTagNameNS(TT_METADATA_NS, TT_OUTPUT_BINDINGS))
        // which contain the standard dmn:Context bindings
        // "standard" in the semi-standard sense of using DMN inside CMMN
  ...
  public static void rewriteInputDataBindingContext(
      Element trisoInputBinding, Element dmnBindings, Document dox) {
    ...
    XMLUtil.migrateNamespace(dox, dmnBindings, DMN_13_XMLNS, DMN_12_XMLNS);
  ...
  protected static void rewriteInputDataBinding(
      Element scopingTask, Element dmnContext, Element varNode, Document dox, XPathUtil x) {
  ...
  private static void addExtension(
      Element scopingElement, Element extensionElement, Document dox, XPathUtil x) {
   ...
   */
  private static Element getScopedBindings(Element dmnContext, Predicate<Element> filter) {
  ...
  private static void processInputContextEntry(
      Element contextExtry,
      Element inputEl,
      Document dox, XPathUtil x) {
   ...
   */
  protected static Optional<Element> findExogenousDMNElement(Element bindingElement) {
    return asElementStream(bindingElement.getChildNodes())
        .flatMap(filterAs(Element.class))
        .filter(el -> DMN_12_XMLNS.equals(el.getNamespaceURI())
            || DMN_13_XMLNS.equals(el.getNamespaceURI())
            || DMN_14_XMLNS.equals(el.getNamespaceURI()))
        .findFirst();
  }
   ...
   */
  protected static Element ensureCMMNExtensions(Document dox, Node rootNode, XPathUtil x) {
    Element extensions = (Element) x.xNode(rootNode, "./cmmn:extensionElements");
    if (extensions == null) {
      extensions = dox.createElementNS(CMMN_11_XMLNS, TTConstants.CMMN_EL_EXTENSIONS);
  ...
  protected static void rewriteOutputDataBindingContext(
      Element trisoOutputBinding, Element dmnBindings, Document dox) {
    XPathUtil x = new XPathUtil();
  ...
  protected static void rewriteOutputDataBinding(
    ...
    var cfi = Optional.of(
            varNode.getAttributeNS(TTConstants.TT_METADATA_NS, "caseFileItemRef"))
